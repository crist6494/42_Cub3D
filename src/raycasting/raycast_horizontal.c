...
/*   Updated: 2023/07/13 18:16:07 by cmorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

static void horizontal_raycast_loop(t_map *map, t_coord *wall_hit_horizontal, t_coord *step, t_ray *data);
...
static void	horizontal_raycast_loop(t_map *map, t_coord *wallHitHorizontal, t_coord *step, t_ray *ray)
{
	int	horizontal_hit;

	horizontal_hit = 0;
	while (horizontal_hit == 0
		&& wallHitHorizontal->x < (float)(map->width)
		&& wallHitHorizontal->x > 1.0 && wallHitHorizontal->y > 1.0
		&& wallHitHorizontal->y < (float)(map->height))
	{
		if (map->tour[(int)wallHitHorizontal->y / (int)map->lim][(int)wallHitHorizontal->x / (int)map->lim] == '1' 
			|| (ray->down == 0 && map->tour[(int)(wallHitHorizontal->y - 1)/ (int)map->lim][((int)wallHitHorizontal->x) / (int)map->lim] == '1'))
			horizontal_hit = 1;
		else
		{

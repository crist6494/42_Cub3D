...
/*   Updated: 2023/07/11 19:41:46 by cmorales         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_horizontal_texture(t_txt_draw * txt, t_ray *ray, t_game *game)
{
	if(ray->down == 1)
	{
		...
		txt->coord_x_txt = (game->map->lim - (int)txt->WallHit->x % (int)game->map->lim) / (float)(game->map->lim * txt->texture->width);
	}
	else
	{
		txt->texture = game->comp->so;
		txt->coord_x_txt = ((int)txt->WallHit->x % (int)game->map->lim) / (float)(game->map->lim * txt->texture->width);
	}
...
void	set_vertical_texture(t_txt_draw * txt, t_ray *ray, t_game *game)
{
	if(ray->left == 1)
	{
		...
		txt->coord_x_txt = (game->map->lim - (int)txt->WallHit->y % (int)game->map->lim) / (float)(game->map->lim * txt->texture->height);
	}
	else
	{
		txt->texture = game->comp->we;
		txt->coord_x_txt = ((int)txt->WallHit->y % (int)game->map->lim) / (float)(game->map->lim * txt->texture->height);
	}
	/* if (txt->coord_x_txt >= (int)game->comp->we->width)
		(txt->coord_x_txt)--; */
}
float	raycast(t_game *game, t_player *player, t_ray *ray, float angle)
{
	...
	{
		insert_coord(player->txt->WallHit, wallHitHorizontal.x, wallHitHorizontal.y);
	...
	{
		insert_coord(player->txt->WallHit, wallHitVertical.x, wallHitVertical.y);
